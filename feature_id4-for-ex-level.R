...
UTR_5_for_upfold_with_symbol = merge(UTR_5_for_gene_up_with_symbol, gene_up[,c('pSILAC Si5B/CTL Hyp ratio','Gene Name')], by.x="hgnc_symbol", by.y="Gene Name")
UTR_5_for_downfold_with_symbol = merge(UTR_5_for_gene_down_with_symbol, gene_down[,c('pSILAC Si5B/CTL Hyp ratio','Gene Name')], by.x="hgnc_symbol", by.y="Gene Name")
UTR_5_for_no_changefold_with_symbol = merge(UTR_5_for_gene_no_change_with_symbol, gene_no_change[,c('pSILAC Si5B/CTL Hyp ratio','Gene Name')], by.x="hgnc_symbol", by.y="Gene Name")

write.csv(UTR_5_for_upfold_with_symbol, "UTR_5_for_info_up.csv", row.names=T)
write.csv(UTR_5_for_downfold_with_symbol , "UTR_5_for_info_down.csv", row.names=T)
write.csv(UTR_5_for_no_changefold_with_symbol, "UTR_5_for_info_no_change.csv", row.names=T)
